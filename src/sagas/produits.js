...
import { SET_PRODUITS, GET_PRODUITS, GET_PRODUITS_STARTED, GET_PRODUITS_FAILED, PUT_PRODUIT, POST_PRODUIT, DELETE_PRODUIT, CANCEL_PRODUIT_UPDATE, EDIT_PRODUIT, SET_EDIT_MODE_PRODUIT } from '../actions';
...
function* workerGetProduits() {
  ...
  try {
    // const uri = 'https://jsonplaceholder.typicode.com/produits';
...
function* workerEditProduit(action) {
  console.log('Editing a produit', action);
    yield put({ type: SET_EDIT_MODE_PRODUIT, value:{produitId: action.value, editMode: true}});
} 

function* workerCancelProduitUpdate(action) {
  console.log('Cancelled a produit edit');
    yield put({ type: SET_EDIT_MODE_PRODUIT, value:{produitId: action.value, editMode: false}});
} 
